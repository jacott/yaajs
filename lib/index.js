...
Context.prototype.loadModule = function (mod) {
  ...
  try {
    vm.runInThisContext(wrap(mod,fs.readFileSync(mod.uri)), {
    ...
    if (ex.code === 'ENOENT') {
      try {
        const result = yaajs.nodeRequire(mod.id);
...
const wrap = (mod, code)=> wrapStart +
      code +
      '\n}(__yaajsVars__.define, __yaajsVars__.nodeRequire, __yaajsVars__.yaajs));';

Context.setGlobalName = value =>{wrapStart = '(function (define, require, '+value+') { '};
