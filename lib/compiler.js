...
module.exports.parse = (codeIn, config)=> terser.minify(codeIn, config);

const compile = module.exports.compile = (config, callback, error)=>{
  const runtimeCtx = yaajs.config(config);
  const compileConfig = config;
  const compileCtx = Module.mainCtx = new Context(config);
  ...
  const parserConfig = Object.assign({}, config.parserConfig);
  let codeTree = {};
  let fileCount = 0;
  ...
  const parse = (codeIn, codeFilename)=> {
    try {
      return terser.minify(codeIn, astConfig).ast;
    } catch (ex) {
  ...
  const _comp = () =>{
    ...
    callback({ast: new terser.AST_Toplevel({body}), code, name});
  ...
  Module.Plugin.prototype.fetch = function (name, parent) {
    ...
    const onWrite = code =>{
      ...
      addCode({ast: parse(code, sfn), filename: sfn, code});
    };
  ...
  const parseContents = (mod, code)=>{
    ...
    ast.walk(new terser.TreeWalker(node => {
      if (node.TYPE === 'Call' &&
        ...
        if (name === undefined) name = new terser.AST_String({start: node.start, end: node.end, value: mod.id});
