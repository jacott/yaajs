...
const newNode = (node, type) => {
  const nn = new node.constructor(void 0, node.start, node.loc);
  nn.type = type;
  nn.end = node.end;
  return nn;
};
...
const compile = module.exports.compile = ({
  ...
  compileCtx.loadModule = function (mod) {
    ...
    try {
      parseContents(mod, fs.readFileSync(mod.uri).toString());
    } catch(ex) {
      throw ex;
    } finally {
  ...
  const addCode = (ast, mod) =>{
    if (toplevel === void 0)
      ...
      const b = ast.body;
      const {body} = toplevel.program === void 0 ? toplevel : toplevel.program;
  ...
  const parseContents = (mod, code)=>{
    if (onBuildRead !== void 0) code = onBuildRead(mod, code).toString();
    ...
    walk(ast, (node) => {
      if (node.type === 'CallExpression' && node.callee.name === 'define') {
        ...
        } else if ((body.type === 'ArrowFunctionExpression' || body.type === 'FunctionExpression')
                   && body.params.length != 0) {
          deps = [];
          node.arguments = [name, null, body];
          body.params.forEach(node => {
            node.type == 'Identifier' && ReservedName[node.name] !== void 0 && addDep(node.name);
          });
