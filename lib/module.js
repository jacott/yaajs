...
function decDepCount(mod) {
  var ctx = mod.ctx;
  --ctx.depCount;
  --mod.depCount || ctx.paused || mod._ready();
}
...
Module._prepareDefine = function(mod, deps, body, autoRequire) {
  ...
  if (deps) {
    ...
    for(var i = 0; i < deps.length; ++i) {
      ...
      depMap[fetchModule(ctx, name, mod).id] = 1;
    }
  }
...
function breakCycle(ctx) {
  while (ctx.waitLoaded === 0 && ctx.depCount !== 0) {
    var link = null;
    for (var wm in ctx.waitDep) {
      var perm = {};
...
function findCycle(mod, temp, perm) {
  ...
  var waitDep = mod.ctx.waitDep;
  var reqs = mod.depMap;
  try {
    for (var id in reqs) {
      if (! id || ! waitDep[id] || ! reqs[id]) continue;
...
Module.prototype = {
  ...
  unload: function () {
    ...
    // FIXME deref depMap
    // FIXME dec ctx.depCount by our this.depCount
    // FIXME delete ctx.waitDep[this.id];
    // FIXME ctx.waitLoaded
    // FIXME rename ctx.waitLoaded to waitingCount
  ...
  handleError: function(error, tracking) {
    var mod = this;
    mod.prevState = mod.state;
    mod.state = ERROR;
    if (tracking) var nested = true;
    else tracking = {}; // me might have cycles
    ...
    try {
      ...
      if (mod.state === ERROR) {
        ...
        for (var id in mod.dependants) {
          var subm = modules[id];
          subm && ! tracking[id] && subm.handleError(error, tracking);
        }
      }
...
Plugin.prototype = {
  ...
  ready: function () {
    ...
    for (var name in resolved) {
      ...
      if (mod.state === READY_WAIT_PLUGIN) {
        mod.state = LOADED;
        ++mod.ctx.depCount; // reverse decDepCount
        decDepCount(mod);
      } else {
        // waiting for name
        mod.id || ++mod.ctx.waitLoaded;
