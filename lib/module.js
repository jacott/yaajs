class Module {
  ...
  unload() {
    ...
    if (this._requires) for (const id in this._requires) {
...
const fetchModule = (ctx, id, parent, callback, error)=>{
  ...
  if (mod !== undefined) {
    ...
    match = /^([^!]+)!(.*)$/.exec(id);
    mod = match ? newResourceModule(ctx, match[1], match[2], parent, named ? id : undefined)
      : new Module(ctx, id);
  ...
  } else if (parent && parent.id && mod._requiredBy[parent.id] === undefined) {
...
const breakCycle = Module.breakCycle = ctx =>{
  while (ctx.resolvingCount === 0 && ctx.loadingCount === 0 && ctx.depCount !== 0) {
    ...
    if (link) {
      ...
      link[0]._requires[link[1].id] = 0;
      link[1]._requiredBy[link[0].id] = 0;
...
const runBody = mod =>{
  ...
  if (mod.requireDeps) {
    ...
    result = mod.body.apply(global, args);
  } else if (mod.exports) {
    result = mod.body.call(global, mod.require, mod.exports, mod);
  } else {
