...
Module.prototype = {
  ...
  _ready: function () {
    ...
    var exportMap = this.ctx._exportMap;
    var exps = this.exports;
    if (exportMap && exps != null) {
      switch(typeof exps) {
      case 'object':
      case 'function':
        var list = exportMap.get(exps);
        if (! list)
          list = [this];
        else
          list.push(this);
  ...
  unload: function () {
    ...
    if (mod._requires) for (var id in mod._requires) {
      var depon = modules[id];
      if (depon)
        delete depon._requiredBy[mod.id];
    }
    ...
    for (var pId in waiting) {
      ...
      for (var name in mods) {
        ...
        if (mod) {
          ...
          for (var id in curr._requiredBy) {
