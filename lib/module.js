...
function breakCycle(ctx) {
  while (ctx.resolvingCount === 0 && ctx.depCount !== 0) {
    var perm = {};
    var link = null;
...
Module.prototype = {
  ...
  _nodefine: function () {
    ...
    if (ctx.requireDefine)
      this._error(this.error('Define not run', 'nodefine'));
    else
...
Plugin.prototype = {
  ...
  ready: function () {
    ...
    for (var name in resolved) {
      ...
      if (mod.state === READY_WAIT_PLUGIN) {
        mod.state = LOADED;
        ++mod.ctx.depCount; // we don't want decDepCount changing this
        decDepCount(mod);
      } else {
        mod.id || ++mod.ctx.resolvingCount; // we were waiting for a name
