...
Module._prepareDefine = function(mod, deps, body, autoRequire) {
  ...
  var oldCount = ctx.depCount === 0 && 0;
  while (ctx.waitLoaded === 0 && ctx.depCount !== oldCount) {
    oldCount = ctx.depCount;
    var wm = null;
    for (wm in ctx.waitDep) break;
    if (! wm) break;
    wm = ctx.waitDep[wm];
    informDependants(wm);
  }
...
function findWaiting(mod, temp, perm) {
  var modId = mod.id;
  if (temp[modId]) return mod;
  ...
  try {
    for (var id in dependants) {
      ...
      if (findWaiting(child, temp, perm))
        return mod;
    }
  } finally {
    perm[modId] = true;
...
Module.prototype = {
  ...
  addDependancy: function (mod) {
    if (! mod.id) return;
    if (this.dependants[mod.id]) return;
    this.dependants[mod.id] = 1;
    if (mod.state === READY || this.state === READY) return;
    ++mod.depCount;
    ++mod.ctx.depCount;
  },
...
function informDependants(mod) {
  ...
  for (var id in mod.dependants) {
    var subm = waitDep[id];
    if (subm) {
      if (subm.state === READY) continue;
      --mod.ctx.depCount;
      --subm.depCount || subm._ready();
    }
  }
  if (mod.callback) {
      ...
      for (var name in mods) {
        ...
        if (mod) {
          ...
          for (var id in curr.dependants) {
            var dep = curr.dependants[id];
            dep && mod.addDependancy(dep);
          }
          if (curr.callback) {
            if (mod.callback)
