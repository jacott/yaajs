...
class Module {
  constructor(ctx, id, state) {
    ...
    if (id) {
      ctx.modules[id] = this;
      if (state === void 0) this.uri = ctx.uri(id);
    }
  }
  ...
  _ready() {
    ...
    this.state = READY;
    this.plugin && this.plugin.ready();
    informDependants(this);
  }

  _nodefine() {
    const {ctx} = this;
    const shim = ctx.shim && ctx.shim[this.id];
  ...
  normalizeId(id) {
    ...
    return ctx.normalizeId(id, id.charAt(0) === "." ? this.dir : void 0);
  }
  ...
  dependOn(id) {
    ...
    if (dMod === void 0 || this.isUnloaded()) return;
    if (this._requires === void 0)
      this._requires = {};
  ...
  unload() {
    ...
    for (const id in this._requiredBy) {
      ...
      subm === void 0 || subm.unload();
    }
    this.uri && ctx.undef(this);
    if (this._unloads !== void 0) for (const cb of  this._unloads) {
  ...
  _error(error, tracking) {
    ...
    try {
      ...
      if (this.state === ERROR) {
        ...
        for (const id in this._requiredBy) {
          const subm = modules[id];
          subm !== void 0 && tracking[id] === void 0 && subm._error(error, tracking);
        }
      }
  ...
  static _prepare(mod, deps, body, autoRequire) {
    ...
    if (deps) {
      for(let i = 0; i < deps.length; ++i) {
        ...
        if (dMod !== void 0 && dMod.id !== '') deps[i] = dMod.id;
        if (mod.isUnloaded()) {
          return;
        }
      }
...
const fetchModule = (ctx, id, parent, callback, error)=>{
  ...
  if (mod !== void 0) {
    ...
    match = /^([^!]+)!(.*)$/.exec(id);
    mod = match ? newResourceModule(ctx, match[1], match[2], parent, named ? id : void 0)
      : new Module(ctx, id);
  ...
  } else if (parent && parent.id && mod._requiredBy[parent.id] === void 0) {
...
const breakCycle = Module.breakCycle = ctx =>{
...
class Plugin {
  ...
  fetch(name, parent, mod) {
    ...
    const parents = this.waiting[parentId] || (this.waiting[parentId] = {});
    const args = parents[name];
    if (args !== void 0) return args[1];
    if (mod === void 0)
  ...
  load(name, mod) {
    ...
    const onLoad = (value)=>{
      ...
      if (value !== void 0) mod.exports = value;
      mod.ctx.waitReady[mod.id] = mod;
