...
describe("Compiling modules", ()=>{
  ...
  it("can compile single require arrow functions", () => {
    let ast;
    compiler.compile({name: "data/min-req-define", contextConfig, callback(r) {ast = r.ast}});
    assert.equal(ast.type, "Program");
    let code = gcode(ast);
    assertAst(ast, [
      `define("data/dep2",["require","exports","module"],(require,exports,module)=>true);`,
      `define("data/min-req-define",["require","data/dep2"],require=>require("./dep2"));`
    ]);
  });
  ...
  const gcode = (node)=> {
    const ans = generate(node, { sourceMaps: false, minified: true, comments: false });
